define([
	...
	c.sendCommand = function(command) {
		...
		if (window.isDebug) {
			...
			var uCommand = "scheme://" + _command.split(",")[0];
			if(_command.replace(/\d*,?/,"") !== "") {
				uCommand += ("?command=" + _command.replace(/\d*,?/,""))
			}

			console.log("native:command: "+uCommand);
		}
		...
		var object = common.doc.createElement('object');
		object.setAttribute('display', 'none');
		object.setAttribute('data', uCommand);

		common.doc.documentElement.appendChild(object);
	...
	var tipsObj = [
		...
		{"type":0,"title":"ドッペル","text":"ドッペルを持つ魔法少女は@マギアゲージが200まで溜まると発動できます"},
		...
		{"type":0,"title":"マギアLv","text":"マギアLvを5にするとマギアゲージ上限が@200まで解放されます"},
	...
	c.purchaseItem = function(model) {
		...
		if(window.isDebug) {
			// debug
			pack = 'jp.f4samurai.madomagi.purchase.item.' + model.moneyCode;
		} else {
			// 本番
			pack = 'com.aniplex.magireco.item.' + model.moneyCode;
		}
	 ...
	 */
	c.getQuestResult = function(object) {
		var json = JSON.stringify(object);
		var command = c.DATA_GET_QUEST_RESULT_JSON + "," + json;
		this.sendCommand(command);
	};
	 ...
	 * ローディング表示
	 * @param {object}  object
	 * 	{object}  object.tips Tipsを表示するために必要な情報
	 */
	 ...
	 */
	c.downloadFile = function(type) {
		var command = c.SCENE_PUSH_DOWNLOAD + "," + type;
		this.sendCommand(command);
	};
	 ...
	 * @param {string} type ダウンロードタイプ(common:全て,voice:ボイスデータ,movie:ムービーデータ)
	 */
	c.downloadFileConfigPage = function(type) {
		var param = {};
			param.category      = type;
	...
	c.startStory = function(storyId,prm) {
		...
		if(common.storage.user && tutorialId == "TU999") {
	 ...
	 * @param {object} object クエスト情報
	 */
	c.startQuest = function(questId,urls) {
		var object = {};
		...
		object.tips = tipsObj[Math.floor(Math.random() * tipsObj.length)];
		var json = JSON.stringify(object);
	...
	c.startArena = function(object) {
		...
		object.record = true;
		var json = JSON.stringify(object);
		// var command = c.SCENE_PUSH_ARENA + "," + json;
		var command = c.SCENE_PUSH_QUEST + "," + json;
		this.sendCommand(command);
	};
	 ...
	 *  {number} object.x タッチ座標
	 *  {number} object.y タッチ座標
	 */
	 ...
	 */
	c.touchMiniChara = function(object) {
		var json = JSON.stringify(object);
		var command = c.DISPLAY_PLAY_MINI_MOTION + "," + json;
		this.sendCommand(command);
	};
	 ...
	 */
	 c.noticeApFullTurnOn = function(){
		...
		// 1秒以上の時通知登録
		// 0のときはキャンセルコマンド
		command = c.NOTI_TURN_ON_AP_FULL + "," + 1;
		this.sendCommand(command);
	};
